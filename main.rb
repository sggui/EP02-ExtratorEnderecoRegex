...
class Extrator
  ...
  def bairro
    ...
    @endereco = @endereco.gsub(regex, '')
    return unless @bairro.to_s.to_i != 0
  ...
  def cidade
    ...
    @endereco = @endereco.gsub(regex, '')
    return unless @cidade.nil? || @cidade.to_s.empty?
  ...
  def cep
    ...
    @cep = 'N/A' if @cep.nil? || @cep.to_s.empty?
    @endereco = @endereco.gsub(regex, '')
  end
...
class Programa
  ...
  def extrair(texto)
    regex = %r{\b(((A|a)venida|(A|a)v)|((R|r)odovia)|((R|r)ua|(R|r))|(E|e)strada|(T|t)ravessa)\.?\s([A-Za-záàâãéèêíïóôõöúçñÁÀÂÃÉÈÍÏÓÔÕÖÚÇÑ0-9 .,\-/|:]+)\s(((AC|AL|AP|AM|BA|CE|DF|ES|GO|MA|MT|MS|MG|PA|PB|PR|PE|PI|RJ|RN|RS|RO|RR|SC|SP|SE|TO))|(\d{5}(-| )\d{3}))\b}
